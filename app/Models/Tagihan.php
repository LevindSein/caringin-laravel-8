...
class Tagihan extends Model
{
    ...
    public static function hitungAir(){
        $dataset = Tagihan::where([['awal_airbersih','!=',NULL],['akhir_airbersih','!=',NULL]])
        ->get();

        $tarif = TarifAirBersih::find(1);

        foreach($dataset as $d){
            $akhir = $d->akhir_airbersih;
            $awal = $d->awal_airbersih;
            ...
            //Update Tagihan
            $d->pakai_airbersih = $pakai_airbersih;
            $d->byr_airbersih = $byr_airbersih;
            $d->pemeliharaan_airbersih = $pemeliharaan_airbersih;
            $d->beban_airbersih = $beban_airbersih;
            $d->arkot_airbersih = $arkot_airbersih;
            $d->sub_airbersih = round($ttl_airbersih);
            $d->ttl_airbersih = round($ttl_airbersih);
            $d->sel_airbersih = round($ttl_airbersih);
            $d->stt_airbersih = 1;
            $d->save();
        }
